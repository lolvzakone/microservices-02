...
| Критерий | NGINX | Kong | Tyk | Traefik |
| :--- | :--- | :--- | :--- | :--- |
| **Маршрутизация** | Гибкая, но ручная настройка | Мощная, управляется через API | С богатым набором правил | С автоматическим обнаружением сервисов |
| **Аутентификация** | Требует внешних модулей | Большой выбор готовых плагинов | Широкий набор встроенных политик | Поддерживается через middleware |
| **Расширяемость** | Ограничена (требует сборки модулей) | Очень высокая (плагины на Lua и др.) | Высокая (плагины на Go, Python, JS) | Да (через middleware на Go) |
| **Производительность** | Высокая, проверенная временем | Очень высокая (на базе NGINX) | Высокая | Высокая (оптимизирован для облака) |
| **Сложность настройки**| Средняя | Средняя (упрощается за счет GUI) | Средняя (есть удобный GUI) | Низкая (идеально для Kubernetes) |
| **Модель распространения**| Полностью Open Source | Open Source ядро + Enterprise | Open Source ядро + Enterprise | Полностью Open Source |
